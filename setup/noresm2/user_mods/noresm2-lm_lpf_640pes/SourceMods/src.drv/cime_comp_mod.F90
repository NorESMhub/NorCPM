...
contains
  ...
  subroutine norcpm_split_comm()
    !! NorCPM, run multiple members in one submit
    !! chdir and split global_comm
    ...
    read(envtmp,*,iostat=ierr)npes    !! if MEMBER_PES not set, npes is 0
                                      !! npes: total PEs for each member
    write(*,*) npes,envtmp
    if (npes.eq.0 .or. mppwidth.lt.2*npes) return !! if npes is 0, then the NorCPM setup will be skipped
  ...
  end subroutine norcpm_split_comm
  ...
  subroutine cime_pre_init1(esmf_log_option)
    ...
    !! NorCPM split global_comm and chdir to each member's rundir
    call norcpm_split_comm()
