...
#ifdef LPFNUDGE
  public::read_restart_lpf_nudging
  public::write_restart_lpf_nudging
#endif
...
#ifdef LPFNUDGE
  real(r8)         :: Nudge_LPF_nday
  real(r8)         :: Nudge_LPF_avg_wgt
  real(r8)         :: Nudge_LPF_Coef
#endif
...
#ifdef LPFNUDGE
  real(r8),allocatable:: LPF_Model_U     (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: LPF_Model_V     (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: LPF_Model_T     (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: LPF_Model_S     (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: LPF_Model_Q     (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: LPF_Model_PS    (:,:)    !(pcols,begchunk:endchunk)
#endif
...
#ifdef LPFNUDGE
  real(r8),allocatable::LPF_Nobs_U (:,:,:) !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable::LPF_Nobs_V (:,:,:) !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable::LPF_Nobs_T (:,:,:) !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable::LPF_Nobs_Q (:,:,:) !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable::LPF_Nobs_PS(:,:)   !(pcols,begchunk:endchunk)
  !! try to keep nudge consistant between restart
  real(r8),allocatable:: rest_Nudge_Ustep (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: rest_Nudge_Vstep (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: rest_Nudge_Sstep (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: rest_Nudge_Qstep (:,:,:)  !(pcols,pver,begchunk:endchunk)
  real(r8),allocatable:: rest_Nudge_PSstep(:,:)    !(pcols,begchunk:endchunk)
#endif
...
contains
...
#ifdef LPFNUDGE
   Nudge_LPF_nday      = 5_r8   !! not in namelist, n day window
   Nudge_LPF_Coef      = 1.0_r8 !! not in namelist
#endif
...
#ifdef LPFNUDGE
   call mpibcast(Nudge_LPF_nday     , 1, mpir8 , 0, mpicom)
   Nudge_LPF_avg_wgt   = 1./(Nudge_LPF_nday*Nudge_Times_Per_Day)
   call mpibcast(Nudge_LPF_avg_wgt  , 1, mpir8 , 0, mpicom)
   call mpibcast(Nudge_LPF_Coef     , 1, mpir8 , 0, mpicom)
#endif
#endif
  ...
  subroutine nudging_init
   ...
   if(masterproc) then
     ...
     !!if(masterproc)write(iulog,*)'blc, currday = ',Day !! debug
     ...
     write(iulog,*) ' '
#ifdef LPFNUDGE
     write(iulog,*) '[LPFNUDGE] Nudge_LPF_nday=',Nudge_LPF_nday
     write(iulog,*) '[LPFNUDGE] Nudge_LPF_avg_wgt=',Nudge_LPF_avg_wgt
     write(iulog,*) '[LPFNUDGE] Nudge_LPF_Coef    =',Nudge_LPF_Coef
     write(iulog,*) ' '
#endif
...
#ifdef LPFNUDGE
     !! TBD: need check run freq, assume it once per day
     !! TBD: need decide where to initial first guess
     !! NEED REVISE !! polluted Nobs_U can use only once !!
     if (all(LPF_Nobs_U.eq.0._r8))then !! all LPF_Nobs_U .eq.0, need a first guess
         write(iulog,*) 'NUDGING: LPF, init LPF_* as current model value.'
         LPF_Nobs_U   = Nobs_U(:,:,:,Nudge_ObsInd(1))
         LPF_Nobs_V   = Nobs_V(:,:,:,Nudge_ObsInd(1))
         LPF_Nobs_T   = Nobs_T(:,:,:,Nudge_ObsInd(1))
         LPF_Nobs_Q   = Nobs_Q(:,:,:,Nudge_ObsInd(1))
         LPF_Nobs_PS  = Nobs_PS (:,:,Nudge_ObsInd(1))
         !!LPF_Nobs_U   = Model_U
         !!LPF_Nobs_V   = Model_V
         !!LPF_Nobs_T   = Model_T
         !!LPF_Nobs_Q   = Model_Q
         !!LPF_Nobs_PS  = Model_PS
         LPF_Model_U  = Model_U
         LPF_Model_V  = Model_V
         LPF_Model_T  = Model_T
         LPF_Model_Q  = Model_Q
         LPF_Model_PS = Model_PS
     end if !! all LPF_Nobs_U .eq.0, need init a first guess
     if(masterproc)then !! blc, debug
         write(iulog,'(a,i4.4,i2.2,i2.2,x,i5.5)')'[LPFNUDGE] Cal Nobs data at ',Year,Month,Day,Sec
         !write(iulog,*)'blc, before averaging LPF_Model_PS = ',LPF_Model_PS(:,begchunk)
         !write(iulog,*)'blc, before averaging     Model_PS = ',Model_PS(:,begchunk)
         !write(iulog,*)'blc, before averaging      Nobs_PS = ',Nobs_PS(:,begchunk,Nudge_ObsInd(1))
     end if
     !! low pass obs
     if(Sec.eq.Nudge_Curr_Sec)then !! 
         LPF_Nobs_U = LPF_Nobs_U(:,:,:)*(1-Nudge_LPF_avg_wgt)+Nobs_U(:,:,:,Nudge_ObsInd(1))*Nudge_LPF_avg_wgt
         LPF_Nobs_V = LPF_Nobs_V(:,:,:)*(1-Nudge_LPF_avg_wgt)+Nobs_V(:,:,:,Nudge_ObsInd(1))*Nudge_LPF_avg_wgt
         LPF_Nobs_T = LPF_Nobs_T(:,:,:)*(1-Nudge_LPF_avg_wgt)+Nobs_T(:,:,:,Nudge_ObsInd(1))*Nudge_LPF_avg_wgt
         LPF_Nobs_Q = LPF_Nobs_Q(:,:,:)*(1-Nudge_LPF_avg_wgt)+Nobs_Q(:,:,:,Nudge_ObsInd(1))*Nudge_LPF_avg_wgt
         LPF_Nobs_PS= LPF_Nobs_PS(:,:) *(1-Nudge_LPF_avg_wgt)+Nobs_PS(:,:,Nudge_ObsInd(1)) *Nudge_LPF_avg_wgt
         !! low pass model variable
         LPF_Model_U =LPF_Model_U *(1-Nudge_LPF_avg_wgt)+Model_U*Nudge_LPF_avg_wgt
         LPF_Model_V =LPF_Model_V *(1-Nudge_LPF_avg_wgt)+Model_V*Nudge_LPF_avg_wgt
         LPF_Model_T =LPF_Model_T *(1-Nudge_LPF_avg_wgt)+Model_T*Nudge_LPF_avg_wgt
         LPF_Model_Q =LPF_Model_Q *(1-Nudge_LPF_avg_wgt)+Model_Q*Nudge_LPF_avg_wgt
         LPF_Model_PS=LPF_Model_PS*(1-Nudge_LPF_avg_wgt)+Model_PS*Nudge_LPF_avg_wgt
         !! replace low pass value from model to obs, which will be Target_U
         !! Nobs_U is 4 dims, Model_U and LPF_*_U are 3 dims.
         !! (pcols,pver,begchunk:endchunk,Nudge_NumObs) and (pcols,pver,begchunk:endchunk)
         if(masterproc) write(iulog,*)'[LPFNUDGE] Update Nobs_* variables.'
         Nobs_U(:,:,:,Nudge_ObsInd(1)) = Model_U(:,:,:)-(LPF_Model_U *Nudge_LPF_Coef)+(LPF_Nobs_U(:,:,:)*Nudge_LPF_Coef)
         Nobs_V(:,:,:,Nudge_ObsInd(1)) = Model_V(:,:,:)-(LPF_Model_V *Nudge_LPF_Coef)+(LPF_Nobs_V(:,:,:)*Nudge_LPF_Coef)
         Nobs_T(:,:,:,Nudge_ObsInd(1)) = Model_T(:,:,:)-(LPF_Model_T *Nudge_LPF_Coef)+(LPF_Nobs_T(:,:,:)*Nudge_LPF_Coef)
         Nobs_Q(:,:,:,Nudge_ObsInd(1)) = Model_Q(:,:,:)-(LPF_Model_Q *Nudge_LPF_Coef)+(LPF_Nobs_Q(:,:,:)*Nudge_LPF_Coef)
         Nobs_PS(:,:,Nudge_ObsInd(1))  = Model_PS(:,:) -(LPF_Model_PS*Nudge_LPF_Coef)+(LPF_Nobs_PS(:,:) *Nudge_LPF_Coef)
     end if !! Sec.eq.Nudge_Curr_Sec
     if(masterproc)then !! blc, debug
         !write(iulog,*)'blc, after  averaging LPF_Model_PS = ',LPF_Model_PS(1,begchunk)
         !write(iulog,*)'blc, after  averaging     Model_PS = ',Model_PS(1,begchunk)
         !write(iulog,*)'blc, after  averaging      Nobs_PS = ',Nobs_PS(1,begchunk,Nudge_ObsInd(1))
     end if
#endif
...
#ifdef LPFNUDGE
     if(Sec.ne.Nudge_Curr_Sec)then !! blc, overwrite nudge values from restart file
        Nudge_Ustep = rest_Nudge_Ustep 
        Nudge_Vstep = rest_Nudge_Vstep 
        Nudge_Sstep = rest_Nudge_Sstep 
        Nudge_Qstep = rest_Nudge_Qstep 
        Nudge_PSstep= rest_Nudge_PSstep
        if(masterproc) write(iulog,*)'LPFNUDGE: overwrite nudge values from restart file, Sec = ',Sec
     end if
#endif
     !******************
     ! DIAG
...
#ifdef LPFNUDGE
!if(masterproc.and.phys_state%lchnk.eq.begchunk) then  !! blc, debuging
!    call get_curr_date(Year,Month,Day,Sec)
!    write(iulog,'(a,g0,x,i0,x,i0)')'blc, before phys_tend%u(1,1) = ', phys_tend%u(1,1),Day,Sec
!end if 
#endif 
   if(Nudge_ON) then
...
#ifdef LPFNUDGE
!if(masterproc.and.phys_state%lchnk.eq.begchunk) then  !! blc, debuging
    !write(iulog,'(a,g0,x,i0,x,i0)')'blc, after  phys_tend%u(1,1) = ', phys_tend%u(1,1),Day,Sec
!end if 
#endif 
...
#ifdef LPFNUDGE
subroutine read_restart_lpf_nudging(File)
    ! read LPF_* from restart
    !!! NB !!! the nudging init after restart reading
    use ppgrid        ,only: pver,pcols,begchunk,endchunk
    use pio, only: file_desc_t, var_desc_t, io_desc_t, pio_double, pio_read_darray
    use cam_grid_support,    only: cam_grid_get_decomp, cam_grid_id , cam_grid_dimensions
    
    type(file_desc_t),pointer,intent(in)  :: File
    !! mod
    ...
    integer :: dims3d(3),dims2d(2), gdims(3),nhdims
    integer :: ierr
    type(var_desc_t):: vdesc
    type(io_desc_t),pointer :: iodesc2d, iodesc3d
    integer :: physgrid
  
    if(masterproc) write(iulog,*) '[LPFNUDGE] Read LPF variables from restart.'

    !! check dims
    dims2d(1) = pcols
    dims2d(2) = endchunk - begchunk + 1
    dims3d(1) = pcols
    dims3d(2) = pver
    dims3d(3) = endchunk - begchunk + 1
    physgrid = cam_grid_id('physgrid') !! although nudging dynamical variabes
    call cam_grid_dimensions(physgrid, gdims(1:2), nhdims)
    gdims(3) = pver
    s2d = pcols*(endchunk-begchunk+1)
    s3d = s2d*pver
    !! get iodesc
    call cam_grid_get_decomp(physgrid, dims2d, gdims(1:2), pio_double, iodesc2d)
    call cam_grid_get_decomp(physgrid, dims3d, gdims     , pio_double, iodesc3d)

    !! allocate output
    call allocate_lpf_variables()
    !! read
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_U',  s3d,tmp);LPF_Nobs_U  = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_V',  s3d,tmp);LPF_Nobs_V  = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_T',  s3d,tmp);LPF_Nobs_T  = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_Q',  s3d,tmp);LPF_Nobs_Q  = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Model_U', s3d,tmp);LPF_Model_U = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Model_V', s3d,tmp);LPF_Model_V = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Model_T', s3d,tmp);LPF_Model_T = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'LPF_Model_Q', s3d,tmp);LPF_Model_Q = reshape(tmp,dims3d)

    call read_restart_with_pio_r8(File,iodesc2d,'LPF_Nobs_PS', s2d,tmp);LPF_Nobs_PS = reshape(tmp,dims2d)
    call read_restart_with_pio_r8(File,iodesc2d,'LPF_Model_PS',s2d,tmp);LPF_Model_PS= reshape(tmp,dims2d)

    call read_restart_with_pio_r8(File,iodesc3d,'Nudge_Ustep',  s3d,tmp);rest_Nudge_Ustep = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'Nudge_Vstep',  s3d,tmp);rest_Nudge_Vstep = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'Nudge_Sstep',  s3d,tmp);rest_Nudge_Sstep = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc3d,'Nudge_Qstep',  s3d,tmp);rest_Nudge_Qstep = reshape(tmp,dims3d)
    call read_restart_with_pio_r8(File,iodesc2d,'Nudge_PSstep', s2d,tmp);rest_Nudge_PSstep= reshape(tmp,dims2d)

    !! debug
    !if(masterproc)then
    !    write(iulog,*)'read_restart_lpf_nudging(): read LPF_Model_PS = ',LPF_Model_PS
    !end if

end subroutine read_restart_lpf_nudging

subroutine read_restart_with_pio_r8(File,iodesc,vn,vsize,var)
    !! write dist array via pio_write_darray()
    use pio, only: file_desc_t, var_desc_t, io_desc_t, &
                   pio_inq_varid, pio_read_darray, &
                   pio_double 
    type(file_desc_t),intent(inout)       :: File
    type(io_desc_t),intent(inout)         :: iodesc ! need be inout for pio_write_darray()
    character(len=*), intent(in)          :: vn
    integer, intent(in)                   :: vsize
    real(r8), intent(out),allocatable     :: var(:) !! 1d
    !! local
    type(var_desc_t):: vdesc
    integer :: ierr
    ...
    !!call err_pio_h(pio_inq_varid(File,trim(vn),vdesc))
    ierr = pio_inq_varid(File,trim(vn),vdesc)
    if (ierr.ne.0)then
        if(masterproc)then
            write(iulog,*)trim(vn),' is not available, skip.'
        end if
        var = 0._r8 !! should be filled with model value when first nudge.
        return
    end if

    !! read from file
    call pio_read_darray(File, vdesc, iodesc, var, ierr)
    call err_pio_h(ierr)
end subroutine read_restart_with_pio_r8

subroutine write_restart_lpf_nudging(File)
    ! write restart
    !   get iodesc from cam_grid_get_decomp()
    !   write with pio_write_darray()
    !!! NB !!! the nudging init after restart reading
    use ppgrid        ,only: pver,pcols,begchunk,endchunk
    use pio, only: file_desc_t, var_desc_t, io_desc_t, pio_freedecomp, pio_double, pio_write_darray &
                  ,var_desc_t,pio_inq_dimid, pio_redef, pio_def_var, pio_enddef
    use cam_grid_support,    only: cam_grid_get_decomp, cam_grid_id , cam_grid_dimensions
    ...
    !! local
    integer :: dims2d(2), dims3d(3), gdims(3), nhdims
    integer :: dimids(3)
    integer :: physgrid
    integer :: i, s2d, s3d
    ...
    if(masterproc) write(iulog,*)'[LPFNUDGE] Write LPF variables from restart.'
    ...
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_U',  dims3d,reshape(LPF_Nobs_U,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_V',  dims3d,reshape(LPF_Nobs_V,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_T',  dims3d,reshape(LPF_Nobs_T,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Nobs_Q',  dims3d,reshape(LPF_Nobs_Q,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Model_U', dims3d,reshape(LPF_Model_U, (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Model_V', dims3d,reshape(LPF_Model_V, (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Model_T', dims3d,reshape(LPF_Model_T, (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'LPF_Model_Q', dims3d,reshape(LPF_Model_Q, (/s3d/)))

    call write_restart_with_pio_r8(File,iodesc2d,'LPF_Nobs_PS', dims2d,reshape(LPF_Nobs_PS, (/s2d/)))
    call write_restart_with_pio_r8(File,iodesc2d,'LPF_Model_PS',dims2d,reshape(LPF_Model_PS,(/s2d/)))

    call write_restart_with_pio_r8(File,iodesc3d,'Nudge_Ustep',  dims3d,reshape(Nudge_Ustep,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'Nudge_Vstep',  dims3d,reshape(Nudge_Vstep,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'Nudge_Sstep',  dims3d,reshape(Nudge_Sstep,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc3d,'Nudge_Qstep',  dims3d,reshape(Nudge_Qstep,  (/s3d/)))
    call write_restart_with_pio_r8(File,iodesc2d,'Nudge_PSstep', dims2d,reshape(Nudge_PSstep,(/s2d/)))

    if(masterproc)then
        write(iulog,*)'write_restart_lpf_nudging(): write LPF_Model_PS = ',LPF_Model_PS
    end if
end subroutine write_restart_lpf_nudging

subroutine write_restart_with_pio_r8(File,iodesc,vn,vdims,var)
    !! write dist array via pio_write_darray()
    use pio, only: file_desc_t, var_desc_t, io_desc_t, &
                   pio_write_darray, &
                   pio_def_var, pio_double &
                   ,pio_redef, pio_enddef, pio_inq_dimid
    type(file_desc_t),intent(inout)  :: File
    type(io_desc_t),intent(inout)         :: iodesc ! need be inout for pio_write_darray()
    character(len=*), intent(in)          :: vn
    integer, intent(in)                   :: vdims(:)
    real(r8), intent(in)                  :: var(:) !! 1d, use reshape()
    !! local
    type(var_desc_t):: vdesc
    integer :: ierr
    integer,allocatable :: dimids(:)

    !! allocate, but vdims is (pcol,pver,chunks), dimids is (lon,lat,lev)
    dimids = vdims
    call err_pio_h(pio_inq_dimid(File,'lon',dimids(1)))
    call err_pio_h(pio_inq_dimid(File,'lat',dimids(2)))
    if (size(dimids).eq.3) call err_pio_h(pio_inq_dimid(File,'lev',dimids(3)))

    call err_pio_h(pio_redef(File))
    call err_pio_h(pio_def_var(File, trim(vn), pio_double, dimids, vdesc))
    call err_pio_h(pio_enddef(File))

    !! write to file
    call pio_write_darray(File, vdesc, iodesc, var, ierr)
    call err_pio_h(ierr)
end subroutine write_restart_with_pio_r8

!!!! tools 
subroutine err_pio_h(ierr)
    !! pio error code handler
    integer, intent(in):: ierr
    if(ierr.eq.0) return
    write(iulog,*) 'NUDGING, err_pio_h() ierr = ',ierr
end subroutine err_pio_h

subroutine allocate_lpf_variables()
    !! allocate LPF releated variables
    !! if already allocated, just return
    use ppgrid        ,only: pver,pcols,begchunk,endchunk
    use error_messages,only: alloc_err
    integer :: n,n3,istat

    if(allocated(LPF_Model_U))return

    n  = pcols*((endchunk-begchunk)+1)  ! 2d var size
    n3 = n*pver                         ! 3d var size
    !! low-pass model variable
    allocate(LPF_Model_U(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Model_U',n3)
    allocate(LPF_Model_V(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Model_V',n3)
    allocate(LPF_Model_T(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Model_T',n3)
    allocate(LPF_Model_S(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Model_S',n3)
    allocate(LPF_Model_Q(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Model_Q',n3)
    allocate(LPF_Model_PS(pcols,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Model_PS',n)
  
    !! low-pass obs variable
    allocate(LPF_Nobs_U(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Nobs_U',n3)
    allocate(LPF_Nobs_V(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Nobs_V',n3)
    allocate(LPF_Nobs_T(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Nobs_T',n3)
    allocate(LPF_Nobs_Q(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Nobs_Q',n3)
    allocate(LPF_Nobs_PS(pcols,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','LPF_Nobs_PS',n)

    !! keep nudge consistant variables
    allocate(rest_Nudge_Ustep(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','rest_Nudge_Ustep',n3)
    allocate(rest_Nudge_Vstep(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','rest_Nudge_Vstep',n3)
    allocate(rest_Nudge_Sstep(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','rest_Nudge_Sstep',n3)
    allocate(rest_Nudge_Qstep(pcols,pver,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','rest_Nudge_Qstep',n3)
    allocate(rest_Nudge_PSstep(pcols,begchunk:endchunk),stat=istat)
    call alloc_err(istat,'nudging_init','rest_Nudge_PSstep',n3)
    ...
    LPF_Model_Q=0._r8
    LPF_Model_PS=0._r8
    rest_Nudge_Ustep = 0._r8
    rest_Nudge_Vstep = 0._r8
    rest_Nudge_Sstep = 0._r8
    rest_Nudge_Qstep = 0._r8
    rest_Nudge_PSstep = 0._r8
end subroutine allocate_lpf_variables

subroutine read_LFPNUDGE_nl()
    !! read the lpfnudge_in if available
    !! and also getenv()
    !! order: namelist -> getenv() -> code default
    !!    since getnev() is set by norcpm script
    character(len=:),allocatable :: nmlfn
    integer :: funit
    logical :: exists
    !!character(len=:),allocatable :: envtmp
    character(len=40) :: envtmp
    namelist /lpfnudge/ Nudge_LPF_nday, Nudge_LPF_Coef

    nmlfn = 'lpfnudge_in'
    inquire(file=nmlfn,exist=exists)

    if(.not. exists)then
        
        if(masterproc) then
            write(iulog,*) '[LPFNUDGE] Namelist '//nmlfn//' is missing, try getenv().'
        end if
        !! try getenv()
        call getenv("Nudge_LPF_nday",envtmp)
        if(len(trim(envtmp)).gt.0)then
            read(envtmp,*)Nudge_LPF_nday 
            if(masterproc) write(iulog,*) '[LPFNUDGE] use env Nudge_LPF_nday = ',Nudge_LPF_nday 
        end if
        call getenv("Nudge_LPF_Coef",envtmp)
        if(len(trim(envtmp)).gt.0)then
            read(envtmp,*)Nudge_LPF_Coef
            if(masterproc) write(iulog,*) '[LPFNUDGE] use env Nudge_LPF_Coef = ',Nudge_LPF_Coef 
        end if
    ...
    !! this uses Fortran 2008 newunit in open()
    open(newunit=funit, file=nmlfn) !!, form="FORMATTED",
    read(funit,nml=lpfnudge)
    close(funit)
    if(masterproc) write(iulog,*) '[LPFNUDGE] Read '//nmlfn//'.'
end subroutine read_LFPNUDGE_nl
