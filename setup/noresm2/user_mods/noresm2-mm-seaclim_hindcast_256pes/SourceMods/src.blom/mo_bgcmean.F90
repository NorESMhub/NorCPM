...
module mo_bgcmean
  ...
  integer, dimension(nbgcmax) ::                                          &
       ...
       & GLB_DOMAIN    =(/0,2,0,0,0,0,0,0,0,0/)
#else
       & GLB_NCFORMAT  =0    ,GLB_INVENTORY =0
#endif
...
#ifdef SEACLIM
       & GLB_NCFORMAT      ,GLB_FNAMETAG      ,GLB_INVENTORY     , GLB_DOMAIN
#else
...
CONTAINS
...
#ifdef SEACLIM
  subroutine msksrf(pos,idepth,domain)
#else 
  subroutine msksrf(pos,idepth)
    ...
    integer, dimension(idm,jdm),save :: mask, mask_n20, mask_ibi
    !
    ...
    if (iniflg) then
      ...
      do j=1,jj
        do i=1,ii
          if (plat(i,j)<20.) mask_n20(i,j)=0
          if (plat(i,j)<20..or.plat(i,j)>80..or.plon(i,j)<-25.0.or.plon(i,j)>30.0) mask_ibi(i,j)=0
          enddo
      enddo
      !$OMP END PARALLEL DO
    endif
    ...
    do j=1,jj
      do l=1,isp(j)
        do i=max(1,ifp(j,l)),min(ii,ilp(j,l))
#ifdef SEACLIM
          if( idepth(i,j)*mask(i,j) <= 0 ) bgcm2d(i,j,pos)=mskval
#else
...
#ifdef SEACLIM
  subroutine msklvl(pos,depths,domain)
#else 
  subroutine msklvl(pos,depths)
...
#ifdef SEACLIM
    integer         :: kmax_tmp(idm,jdm)
    integer, save   :: kmax_n20(idm,jdm), kmax_ibi(idm,jdm)
#endif
    real, parameter :: mskval=nf90_fill_double
    !
    ! --- Check whether field should be processed
    ...
    if (iniflg) then
      ...
      do j=1,jj
        do i=1,ii
          if (plat(i,j)<20.) kmax_n20(i,j)=0
          if (plat(i,j)<20..or.plat(i,j)>80..or.plon(i,j)<-25.0.or.plon(i,j)>60.0) kmax_ibi(i,j)=0
        enddo
      enddo
      !$OMP END PARALLEL DO      
#endif
