...
LOGFILE=$SETUPROOT/../../logs/remove_restarts-`date +%Y%m%d%H%M`_${EXPERIMENT}.log
...
do
  ...
  do
    if [ `find . -samefile $REF_PATH/$ITEM | wc -l` -gt 0 ]
    then
      echo $REF_PATH/$ITEM $EXEROOT/run/$ITEM are same
      rm $EXEROOT/run/$ITEM 
    fi 
  done
