...
!
! !INTERFACE:
  subroutine restFile_write( file, nlend, noptr, rdate , da)
!
! !DESCRIPTION:
...
! !USES:
    ...
    use BiogeophysRestMod, only : BiogeophysRest, BiogeophysRestDA
#if (defined CN)
...
! !ARGUMENTS:
    ...
    logical,           intent(in), optional :: da    ! for da
!
! !CALLED FROM:
...
!-----------------------------------------------------------------------
    ...
    if (present(da))then
      call BiogeophysRestDA( ncid, flag='define' )
    else
    ...
    if (present(da))then
    call BiogeophysRestDA( ncid, flag='write' )
    else
    ...
    if ( ptrfile .and. not(present(da)) ) call restFile_write_pfile( file )
...
!
! !INTERFACE:
  subroutine restFile_read( file, da )
!
! !DESCRIPTION:
...
!-----------------------------------------------------------------------
    ...
    if (present(da))then
    call BiogeophysRestDA( ncid, flag='read' )
    else
