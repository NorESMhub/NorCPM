...
# experiment settings
: ${EXPERIMENT:=norcpm1-1_piControl} # case prefix, not including _YYYYMMDD_memXX suffix 
: ${MEMBER1:=01} # first member  
: ${ENSSIZE:=01} # number of members 
: ${COMPSET:=N1850AERCNOCCMIP6V1}
: ${RES:=f19_g16}
...
: ${RESTART:=9} # restart this many times (perform assimilation three times before forecast) 
: ${WALLTIME:='96:00:00'}
...
: ${VERBOSE:=0} # set -vx option in all scripts
