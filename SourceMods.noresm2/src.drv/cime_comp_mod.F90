...
contains
  ...
  subroutine cime_pre_init1(esmf_log_option)
    ...
    !! need be revised, declare variables, getenv of mppwidth and npes
        ...
        read(envtmp,*)npes
    print*,'blc, debugging, 624; npes: ',npes
        ...
        if (mppwidth.ge.2*npes) then
         ...
         call MPI_Get_processor_name(nodename,reslen,ierr)
         write(*,'(a,i4,i4,i4,a,x,a)')"INFO: myid,rank_local,mem,rundir,nodename: ",myid,rank_local,mem+1,' '//trim(rundir),' '//trim(nodename)
        call mpi_comm_rank(global_comm,myid,ierr)
        call mpi_comm_size(global_comm,mppwidth,ierr)
        print*,'blc, 650; myid,mppwidth:',myid,mppwidth
        endif
    !--- NorCPM modify end
