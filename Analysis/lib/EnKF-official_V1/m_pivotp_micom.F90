...
subroutine pivotp_micom_new(lon, lat,modlon,modlat, ipiv, jpiv, nx, ny, min_r, max_r, &
  ...
  if (jpiv == ny) then
     ...
     if (inside) then
        ipiv = i !its(itw(i,j), jtw(i,j))
        jpiv = j !jts(itw(i,j), jtw(i,j))
        return
     end if
     xx(1) = modlon(i, j)
     ...
     call check_point(lon,lat,xx,yy,inside)
     if (inside) then
        ipiv = itn(ite(i,j), jte(i,j))
        jpiv = jtn(ite(i,j), jte(i,j))
        return
     end if
     ! check south-west
     xx(1) = modlon(i, j)
...
subroutine check_point(x, y, xx, yy, inside)
  ...
  s = (a + b + c) / 2
  sum_triangle_area = sum_triangle_area + sqrt(max(0.,s*(s-a)*(s-b)*(s-c)))
 ...
 ! fourth trangle area with (xx(1), yy(1)), (xx(4), yy(4)) and (x,y)
  ...
  s = (a + b + c) / 2
  rectangle_area = rectangle_area + sqrt(max(0.,s*(s-a)*(s-b)*(s-c)))
