...
use nfw_mod
   ...
   integer :: i,j,k
   real :: dpsum
   real :: ttmp,stmp,dps,q,tdps,sdps
...
!
   ...
   do j=1,jdm
      do i=1,idm
         if (temp(i,j,1,1)<100000.) then
            !dps=dp(i,j,2,1)+dp(i,j,1,1) This should be the case , but to be
            !consistent with convec.f we only use layer 2.
            dps=dp(i,j,2,1)
            tdps=temp(i,j,2,1)*dp(i,j,2,1)
            sdps=saln(i,j,2,1)*dp(i,j,2,1)
            k=kfpla(i,j,1,1)
            ttmp=temp(i,j,2,1)
            stmp=saln(i,j,2,1)
            do while (rho(dps,ttmp,stmp) .gt. rho(dps,temp(i,j,k,1),saln(i,j,k,1)) .or. dp(i,j,k,1) .lt. epsil) 
                tdps=tdps+temp(i,j,k,1)*dp(i,j,k,1)
                sdps=sdps+saln(i,j,k,1)*dp(i,j,k,1)
                dps=dps+dp(i,j,k,1)
                q=1/dps
                ttmp=tdps*q
                stmp=sdps*q
