...
!
! Modifications:
!                17/04/2018 Madlen KIMMRITZ: 
!                    - modified get_local_obs() for handling of ICE data
!                       - we preserve former requirement/handling, 
!                         that when T,S  profile is read it needs to be read first and then SST.
!                         In case of too many observations of T,S and SST: the SST and farest 
!                         T,S prof values are not considered
!                       - additionally we enter ICEC (it treats SST obs as for T,S profiles)
!                       - bit identical results for V1 compared to former get_local_obs()     
    ...
    do o = 1, nobs
       ...
       if (dist(o) < rmax) then
         if (trim(obs(o) % id) .eq. 'TEM' .or. trim(obs(o) % id) .eq. 'SAL') then
             ...
             lobs_pro (npro) = o
             dist_pro (o)    = dist(o)
         elseif (trim(obs(o) % id) .eq. 'ICEC' .or. trim(obs(o) % id) .eq. 'ICET') then
             nicec           = nicec+1
             lobs_ice(nicec) = o
             dist_ice(o)     = dist(o)
