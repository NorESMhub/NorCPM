...
use m_spherdist
  ...
  allocate(F_jpiv(dlon,dlat))
   ...
   call nfw_def_dim('pivots_SST.nc', ncid, 'x', dlon, dimids(1))
   call nfw_def_dim('pivots_SST.nc', ncid, 'y', dlat, dimids(2))
   call nfw_def_var('pivots_SST.nc', ncid, 'ipiv',nf_double, 2, dimids, vIPIV_ID)
   call nfw_def_var('pivots_SST.nc', ncid, 'jpiv',nf_double, 2, dimids, vJPIV_ID)
  call nfw_enddef('pivots_SST.nc', ncid)
  call nfw_put_var_double('pivots_SST.nc', ncid, vIPIV_ID, F_ipiv(:,:))
  call nfw_put_var_double('pivots_SST.nc', ncid, vJPIV_ID, F_jpiv(:,:))
