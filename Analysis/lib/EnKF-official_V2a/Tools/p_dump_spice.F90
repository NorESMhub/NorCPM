program dump_spice
  ...
  integer :: ncid, x_ID, y_ID, z_ID,t_ID
  integer :: dimids(3)
  ...
  integer :: idm,jdm,kdm
  real, allocatable, dimension(:,:,:,:)   :: temp,saln,sp
  ...
  call nfw_inq_dimid(trim(oldfile), ncid, 'time', t_ID)
  !nb total of track
  ...
  !call nfw_inq_dimlen(trim(oldfile), ncid, t_ID, t)
  ...
  ! print *,'compute spiciness'
   ...
   do k=1,kdm
      ...
      if (saln(i,j,k,1)<999999.) then
       sp(i,j,k,1)=spice(temp(i,j,k,1),saln(i,j,k,1))
    else
       sp(i,j,k,1)=saln(i,j,1,1)
      end if
   end do
   end do
   end do
    ...
    call nfw_def_var(trim(oldfile), ncid, 'spice', nf_double, 4, nc(1:4), VSPI_ID)
