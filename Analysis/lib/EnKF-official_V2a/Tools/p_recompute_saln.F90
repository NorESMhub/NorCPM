program recompute_saln
  ...
  integer :: dimids(3)
  logical          :: ex
  integer :: vTEM_ID, vSAL_ID,VSPI_ID,i,j,k
  integer :: idm,jdm,kdm
  ...
  call nfw_inq_dimlen(trim(oldfile), ncid, z_ID, kdm)
   ...
   call nfw_inq_varid(trim(oldfile), ncid,'spice',vSPI_ID)
   call nfw_get_vara_double(trim(oldfile), ncid, vSPI_ID, ns(1:3),nc(1:3),sp)
   ...
   call nfw_put_vara_double(trim(oldfile), ncid, vSAL_ID, ns, nc, saln)
   call nfw_close(trim(oldfile), ncid)
end program recompute_saln
