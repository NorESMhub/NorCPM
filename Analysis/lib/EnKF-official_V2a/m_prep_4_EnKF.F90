...
!Read the monthly mean 
                call get_micom_fld_new('mean_mod', field2, 'fice', 0, 1, nx, ny)
                readfld = readfld - (field2/100.)
#endif
             call Generate_element_Si(S(:, iens), unique_obs(iuobs),&
                  readfld, depths, nx, ny, nz, 0) 
          end do
       elseif (trim(unique_obs(iuobs)) == 'ICET') then
          do iens = 1, nrens
             write(cmem,'(i3.3)') iens
               readfld(:,:)=0.
             do m = 1, ncat
!                print *,'We are reading category',m,trim('forecast_ice'//cmem)
                ! BIPOL/TRIPOL - dependent in the routine
                call get_micom_fld_ice(trim('forecast_ice'//cmem), field2,'aicen', m, m, nx, ny)
                call get_micom_fld_ice(trim('forecast_ice'//cmem), field3,'vicen', m, m, nx, ny)
                readfld=readfld+(field2*field3)
             end do

#ifdef ANOMALY
!Read the monthly mean
                call get_micom_fld_new('mean_mod_ice', field2, 'hi', 0, 1, nx, ny)
                readfld = readfld - field2
#endif
             call Generate_element_Si(S(:, iens), unique_obs(iuobs),&
                  readfld, depths, nx, ny, nz, 0)
