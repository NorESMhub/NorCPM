module m_read_micom_icec

contains

  subroutine read_micom_ICEC(fname,cmonth,data,modlon, modlat, depths,nx,ny,nrobs)
  use mod_measurement
  use mod_grid
  use nfw_mod
...
!  real(4) :: vsst(nx,ny,12)
  real(4) :: vfice(nx,ny)
  integer :: ncid,vFICE_ID,i,j,k,imonth
...
! Read  observation file
  ...
  call nfw_get_att_real(fname, ncid, vFICE_ID, 'add_offset', addoffset)
  call nfw_get_att_real(fname, ncid, vFICE_ID, 'scale_factor', scalefac)
  call nfw_close(fname, ncid)
  k=1
  do j = 1, ny
     do i = 1, nx
      if (depths(i,j)>0) then 
        !=== we scale ice concentration such that in [0,1]
        data(k)%d = (vfice(i,j)*scalefac(1)+addoffset(1))/100.0
        data(k)%ipiv = i
        data(k)%jpiv = j
  ...
  enddo    ! nx
  nrobs=k-1
end subroutine read_micom_ICEC
end module m_read_micom_icec
