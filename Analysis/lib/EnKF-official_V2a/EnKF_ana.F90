  ...
  do m1 = my_first_iteration, my_last_iteration, NFIELD
     ...
     do m = m1, m2
        ...
        do k = 1, ENSSIZE
           ...
           call get_micom_fld_new(trim(memfile), readfld, k, fieldnames(m),&
                fieldlevel(m), 1, idm, jdm)
           ! reshaping and conversion to real(4)
           if ( fieldlevel(m) >=3 .and. trim(fieldnames(m)) /= 'dp' ) then
              if ( trim(fieldnames(m)) /= 'dp' ) then
                 call get_micom_fld_new(trim(memfile), readfld2, k,'dp',&
                      fieldlevel(m), 1, idm, jdm)
                 ! reshaping and conversion to real(4)
                 do jj=1,jdm 
                    do ii=1,idm 
                       fld(ii+(jj-1)*idm, ENSSIZE * (m - m1) + k) = readfld2(ii,jj)*readfld(ii,jj)
                    enddo
                 enddo
              else !dp below the MLD: Anamorphosis (Bertino 2003)
                 do jj=1,jdm 
                    do ii=1,idm 
                       fld(ii+(jj-1)*idm, ENSSIZE * (m - m1) + k) = log(readfld(ii,jj))
                    enddo
                 enddo

              else !2d variables or 3d from MLD
