...
!                20/07/2017 MK - extended for ice concentration
!                              - available set handling: HadISST, micom data 
!                                plus version from YW
!                                OPEN: If Had has no ICEC: read_ice is called anyhow
...
program p_prep_obs
  ...
  use m_read_HadI_SST
  use m_read_NOAA_SST
  use m_read_NOAA_ICEC
  use m_read_HadI_ICEC  
  use m_read_C2SMOS_ICET
  use m_read_ENVISAT_ICET
  ...
  if (trim(producer) == 'Reynolds') then
     ...
     else if (trim(obstype) == 'ICEC') then
        print * ,'Reading HadI ICEC data'
        call read_HadI_ICEC(fname,fnamehdr,data,modlon, modlat, depths,360,180, nrobs)
     else
        stop 'ERROR: (HadI) only produces SST and CICE'
     endif

     allocate(obs(nrobs))
     data_eq_obs = .false.
     print *,'HadI Data read',nrobs
  else if (trim(Producer) == 'C2SMOS') then
     if (trim(obstype) == 'ICET') then
        dosuperob = .true.
        gr%reg = .true.
        gr%order = 2
        gr%nx=432
        ...
        grpoints = gr%nx*gr%ny
        allocate(data(grpoints))
        print * ,'Reading C2SMOS ICET data'
        call read_C2SMOS_ICET(fname,data,modlon, modlat, depths,432,432, nrobs)
        allocate(obs(nrobs))
        print * ,'C2SMOS ICET data read'
     else
        stop 'ERROR: (C2SMOS) only produces ICET'
     endif
  else if (trim(Producer) == 'ENVISAT') then
     if (trim(obstype) == 'ICET') then
        dosuperob = .true.
        gr%reg = .true.
        gr%order = 2
        ...
        grpoints = gr%nx*gr%ny
        allocate(data(grpoints))
        print * ,'Reading ENVISAT ICET data'
        call read_ENVISAT_ICET(fname,data,modlon, modlat, depths,432,432, nrobs)
        allocate(obs(nrobs))
        data_eq_obs = .false.
        print * ,'ENVISAT ICET data read'
     else
        stop 'ERROR: (ENVISAT) only produces ICET'
     endif
     ...
     if (trim(obstype) == 'SST') then
        ...
        print * ,'Reading NOAA ICEC data'
        call read_NOAA_ICEC(fname,fnamehdr,data,modlon, modlat, depths,360,180, nrobs)
     else
     ...
     elseif (trim(obstype) == 'ICEC') then
        call read_micom_ICEC(fname,fnamehdr,data,modlon, modlat, depths, nx, ny, nrobs)
     else
        stop 'ERROR: micom only produces SST'
     endif
     data_eq_obs = .true. !locally
     print *,'Data read'
